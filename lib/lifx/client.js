 ...
 * @param {String[]} [options.lights] List of ip-addresses for the lights trying to control
 ...
 * @param {String[]} [lights] List of ip-addresses for the lights trying to control
 */
Client.prototype.startDiscovery = function(lights) {
  var defaults = ['255.255.255.255'];

  lights = lights || {};
  var opts = _.defaults(lights, defaults);
  var sendDiscoveryPacket = function() {
    // Sign flag on inactive lights
    _.forEach(this.devices, _.bind(function(info, address) {
    ...
    // Send the discovery packet to the lights
    _.forEach(opts, function(light) {
