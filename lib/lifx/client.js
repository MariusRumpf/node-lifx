...
Client.prototype.init = function(options, callback) {
  var defaults = {
    ...
    // If a bulb hasn't been seen for given amount of discoveries it will be removed
    removeLightTolerance: 3
  };
  ...
  this.removeLightTolerance = opts.removeLightTolerance;
...
Client.prototype.startDiscovery = function() {
  var sendDiscoveryPacket = function() {
    // Remove longer inactive gateways
    _.forEach(this.gateways, function(info, address) {
      var diff = this.discoveryPacketSequence - info.seenOnDiscovery;
      if (diff >= this.removeLightTolerance) {
        delete this.gateways[address];
        this.emit('bulb-remove', info);
      }
    }, this);
