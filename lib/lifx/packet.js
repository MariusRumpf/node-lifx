...
Packet.headerToObject = function(buf) {
  ...
  obj.responseRequired = (frameAddressDescription & constants.RESPONSE_REQUIRED_BIT) !== 0;
  offset += 1;
...
Packet.headerToBuffer = function(obj) {
  ...
  if (obj.responseRequired !== undefined && obj.responseRequired === true) {
 ...
 * @return {Buffer} the packet
 */
Packet.toBuffer = function(obj) {
  var headerBuf = this.headerToBuffer(obj);
  ...
  if (obj.type !== undefined) {
    ...
    if (packets[obj.type] !== undefined) {
      if (typeof packets[obj.type].toBuffer === 'function') {
        var specificBuf = packets[obj.type].toBuffer(obj);
        return Buffer.concat([headerBuf, specificBuf]);
      }
    }
  }
 ...
 * @param  {String} (source) the source of the packet
 * @param  {String} (target) the target of the packet
...
Packet.create = function(type, params, source, target) {
  ...
  if (type !== undefined) {
    ...
    if (typeof type === 'string') {
      ...
      obj.type = _.result(_.find(this.typeList, {id: type}), 'id');
    }
    if (obj.type === undefined) {
      return false;
    }
  ...
  obj.addressable = packets[type].addressable;
